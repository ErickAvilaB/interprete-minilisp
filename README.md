...
```bash
((letrec (sum1_n (lambda (n) (if0 n 0 (+ n (sum1_n (sub1 n)))))) sum1_n) 12)   -- 78
```

2. Factorial

```bash
((letrec (fact (lambda (n) (if0 n 1 (* n (fact (sub1 n)))))) fact) 6)   -- 720
```

3. Fibonacci

```bash
((letrec (fib (lambda (n) (if (<= n 1) n (+ (fib (sub1 n)) (fib (sub1 (sub1 n))))))) fib) 10)   -- 55
```
...
```bash
((letrec (map_n (lambda (f xs n) (if0 n [] ((f (head xs)), (map_n f (tail xs) (sub1 n)))))) map_n) (lambda (x) (* x x)) [1, 2, 3, 4] 4)   -- [1,  4,  9,  16]
```
...
```bash
((letrec (filter_n (lambda (p xs n) (if0 n [] (if (p (head xs)) ((head xs), (filter_n p (tail xs) (sub1 n))) (filter_n p (tail xs) (sub1 n)))))) filter_n) (lambda (x) (> x 3)) [1, 2, 3, 4, 5, 6] 6)   -- [4,  5,  6]
```
